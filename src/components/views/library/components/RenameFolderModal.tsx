...
export const RenameFolderModal: React.FC<RenameFolderModalProps> = ({
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    if (!trimmed) {
      setError(t('folderMenu.renameFolderPrompt', 'Bitte gib einen Ordnernamen ein.'));
      return;
    }
    ...
    try {
      ...
      setError(String(err?.message || err || 'Fehler beim Umbenennen des Ordners'));
    } finally {
      setIsSubmitting(false);
    }
  };
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-semibold text-txt-primary">
                {t('folderMenu.renameFolderTitle', 'Ordner umbenennen')}
              </h3>
              <p className="text-xs text-txt-tertiary truncate max-w-[220px]" title={currentName}>
                «{currentName}»
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          <div>
            <label className="block text-xs font-medium text-txt-secondary mb-1.5">
              {t('folderMenu.renameFolderPrompt', 'Neuer Name für den Ordner:')}
            </label>
            <input
              ref={inputRef}
              type="text"
          ...
          <div className="flex items-center justify-end gap-2 pt-1">
            ...
            >
              ...
              <span>{t('folderMenu.renameBtn', 'Umbenennen')}</span>
            </button>
          </div>
        </form>
      </div>
