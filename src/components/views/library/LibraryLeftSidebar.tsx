...
import { Bookmark, ChevronDown, Plus, HardDrive, Folder, ChevronRight, FolderOpen, Sparkles, Loader2, Camera, RefreshCw, Images } from "lucide-react";
  ...
  for (const folder of allFolders) {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (!child) {
        ...
        // Upgrade casing if new name has uppercase/mixed-case letters and existing is all lowercase
        if (part !== part.toLowerCase() && child.name === child.name.toLowerCase()) {
          child.name = part;
          child.path = currentPath + '/' + part;
        }
      }
      current = child;
  ...
  for (let idx = 0; idx < images.length; idx++) {
    ...
    for (let i = 0; i < end; i++) {
      ...
      if (!child) {
        ...
        // Real image file on disk has authoritative folder name
        if (child.fileCount === 0 || (part !== part.toLowerCase() && child.name === child.name.toLowerCase())) {
          child.name = part;
          child.path = currentPath + '/' + part;
        }
      }
...
export function LibraryLeftSidebar() {
  ...
  const collectionsTreeRef = useRef<CollectionsTreeRef>(null);
  ...
  const isSubfolderSelected = useMemo(() => {
    if (selectedFolderPaths && selectedFolderPaths.size > 0) {
      if (selectedFolderPaths.size === 1 && normRoot && selectedFolderPaths.has(normRoot)) {
        return false;
      }
      return true;
    }
    if (activeFolderPath && normRoot && normalizePath(activeFolderPath) !== normRoot) {
      return true;
    }
    return false;
  }, [selectedFolderPaths, activeFolderPath, normRoot]);

  const isEntireArchiveActive = !activeCollectionId && !isViewingLastImport && !isSubfolderSelected && !!rootPath;

  const handleSelectEntireArchive = () => {
  ...
  return (
    <div className="w-full h-full bg-app-panel flex flex-col min-h-0 overflow-hidden">
      ...
      <div data-tour="library-collections" className="flex flex-col flex-shrink-0">
        <div 
          className="px-4 py-3 border-b border-app-border flex items-center justify-between cursor-pointer hover:bg-app-hover/50 transition-colors select-none" 
          onClick={() => setCollectionsOpen(!collectionsOpen)}
        >
          <div className="flex items-center gap-2">
            <ChevronDown className={`w-3.5 h-3.5 text-txt-tertiary transition-transform duration-150 ${!collectionsOpen ? "-rotate-90" : ""}`} />
          ...
          <button
            data-tour="collection-export"
            onClick={(e) => {
              e.stopPropagation();
              if (!collectionsOpen) setCollectionsOpen(true);
              collectionsTreeRef.current?.openCreateAlbum();
            }}
      ...
      <div data-tour="library-folder-tree" className="flex flex-col flex-1 min-h-0">
        <div 
          className="px-4 py-3 border-b border-app-border flex items-center justify-between cursor-pointer hover:bg-app-hover/50 transition-colors flex-shrink-0 select-none" 
          onClick={() => setLibraryOpen(!libraryOpen)}
        >
          <div className="flex items-center gap-2">
            <ChevronDown className={`w-3.5 h-3.5 text-txt-tertiary transition-transform duration-150 ${!libraryOpen ? "-rotate-90" : ""}`} />
            <h2 className="text-xs font-semibold text-txt-secondary uppercase tracking-wider flex items-center gap-2">
        ...
        <div className={`flex flex-col flex-1 min-h-0 ${!libraryOpen ? "hidden" : ""}`}>
          ...
          {/* Entire Archive (Sticky Root) */}
          {rootPath && (
            <div className="px-2 pt-2 pb-1 border-b border-app-border/40 flex-shrink-0">
              <button
                onClick={handleSelectEntireArchive}
                onContextMenu={(e) => {
                  if (tree) {
                    e.preventDefault();
                    e.stopPropagation();
                ...
                className={`w-full flex items-center justify-between px-2.5 py-1.5 rounded-lg text-xs transition-colors cursor-pointer select-none ${
                  isEntireArchiveActive 
                    ? 'bg-accent/20 text-accent font-semibold' 
                    : 'text-txt-primary hover:bg-app-hover'
                }`}
                title={t('sidebar.entireArchiveTooltip', 'Gesamtes Archiv anzeigen (Esc)')}
              >
                <div className="flex items-center gap-2 truncate">
                  <Images className={`w-3.5 h-3.5 flex-shrink-0 ${isEntireArchiveActive ? 'text-accent' : 'text-txt-secondary'}`} />
                  <span className="truncate">{t('sidebar.entireArchive', 'Gesamtes Archiv')}</span>
                </div>
                <span className="text-[10px] text-txt-tertiary font-mono flex-shrink-0">
                  ({imageCount})
                </span>
              </button>
            </div>
